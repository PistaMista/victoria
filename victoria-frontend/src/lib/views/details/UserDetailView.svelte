<script lang="ts">
	...
	let adminRoleChecked: boolean = false;
	$: modifiedUser.role = adminRoleChecked ? "admin" : "user";

	let actions: [Action, boolean][] = [];
	let triggers: [TriggerListItem, boolean][] = [];

	$: modifiedUser.permittedActions = actions
		.filter(([_, e]) => e)
		.map(([v, _]) => v.id);
	$: modifiedUser.permittedTriggers = triggers
		.filter(([_, e]) => e)
		.map(([v, _]) => v.id);
	...
	async function load() {
		if (id !== null) {
			...
			adminRoleChecked = modifiedUser.role === "admin";
		}
		actions = (await getAllActions()).map((val) => [
			val,
			loadedUser.permittedActions.includes(val.id),
		]);
		triggers = (await getAllTriggers()).map((val) => [
			val,
			loadedUser.permittedTriggers.includes(val.id),
		]);
	}
		...
		>
			...
			>
				<DetailView backRoute="/admin/users">
					...
					<DetailViewSection
						title="Permitted agent triggers"
					>
						<div class="flex flex-col space-y-2">
							{#each triggers as trigger}
								<Toggle
									label={trigger[0].name}
									aria-label={trigger[0]
										.name}
