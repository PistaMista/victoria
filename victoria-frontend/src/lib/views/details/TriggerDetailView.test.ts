...
test("trigger detail shows name, type, parameters and template of trigger", async () => {
  ...
  const nameBox = within(await findByLabelText("Name")).getByRole(
    "textbox",
  ) as HTMLInputElement;
  const typeDropdown = getByLabelText("Type");
  const urlBox = within(getByLabelText("URL")).getByRole(
    "textbox",
  ) as HTMLInputElement;
  const intervalBox = within(getByLabelText("Interval")).getByRole(
    "textbox",
  ) as HTMLInputElement;
...
test("trigger detail shows and can edit Poll trigger settings", async () => {
  ...
  const urlBox = within(await findByLabelText("URL")).getByRole(
    "textbox",
  ) as HTMLInputElement;
  const intervalBox = within(getByLabelText("Interval")).getByRole(
    "textbox",
  ...
  expect(updateTriggerHandler).toBeCalled();
  expect((updateTriggerHandler as Mock).mock.calls[0][0].request.url).toContain(
    "/api/triggers/2",
  );
  let body = await (
    updateTriggerHandler as Mock
...
test("trigger detail shows and can edit Chat trigger settings", async () => {
  ...
  const receiverBox = within(await findByLabelText("Chat receiver")).getByRole(
    "textbox",
  ) as HTMLInputElement;
  const saveButton = getByLabelText("Save trigger");
...
test("trigger detail shows and can edit Timer trigger settings", async () => {
  ...
  const intervalBox = within(await findByLabelText("Interval")).getByRole(
    "textbox",
  ) as HTMLInputElement;
  const saveButton = getByLabelText("Save trigger");
...
test("trigger detail can change Poll trigger to Chat trigger", async () => {
  ...
  {
    // Set receiver
    const receiverBox = within(getByLabelText("Chat receiver")).getByRole(
      "textbox",
    ) as HTMLInputElement;
    await user.clear(receiverBox);
    await user.type(receiverBox, "research");
