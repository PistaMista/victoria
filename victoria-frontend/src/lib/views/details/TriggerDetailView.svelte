<script lang="ts">
	...
	let pollVars = ["content"];
	let chatVars = ["message", "chatId", "exchangeId"];
	let webhookVars = ["payload"];

	$: templateEditorVars =
		modifiedTrigger.settings.type == "poll"
			? pollVars
			: modifiedTrigger.settings.type == "chat"
				? chatVars
				: modifiedTrigger.settings.type == "webhook"
					? webhookVars
					: [];
			...
			>
				<DetailView backRoute="/admin/triggers">
					...
					<DetailViewSection title="Template">
						<EventTemplateEditor
							variables={templateEditorVars}
							bind:template={
								modifiedTrigger.template
							}
						/>
					</DetailViewSection>
