<script lang="ts">
    ...
    import { createAgent, deleteAgent, getAgent, updateAgent } from "$lib/api/agents";
    ...
    $: modifiedAgent.enabledActions = actions.filter(([_, e]) => e).map(([v, _]) => v.id);
    ...
    async function initModelOptions() {
        ...
        if (id !== null) {
            chosenModel = modelDropdownOptions.filter(
                (val) => val.value === loadedAgent.baseModelId
            ).at(0) ?? null;
        }
    }
    ...
    async function initActionToggles() {
        actions = (await getPermittedActions()).map(
            (val) => [val, loadedAgent.enabledActions.includes(val.id)]
        )
    }
    
    async function initTriggerToggles() {
        triggers = (await getPermittedTriggers()).map(
            (val) => [val, loadedAgent.enabledTriggers.includes(val.id)]
        )
    }
    ...
    async function setThumbnail(event: Event) {
        ...
        modifiedAgent.thumbnailDataURI = await convertToDataURI(input.files[0]);
    }
...
<DetailView title="{modifiedAgent.name} agent">
    ...
    <DetailViewSection title="System prompt">
        <Textarea
            ...
            bind:value={modifiedAgent.systemPrompt}
        />
    </DetailViewSection>
    ...
    <DetailViewSection title="Model parameters">
        <ParameterSlider
            ...
            min={0.0}
            max={2.0}
            bind:value={modifiedAgent.modelParameters.temperature}
        />
        <ParameterSlider
            label="Top K"
            aria-label="Top K"
            ...
            min={0}
            max={1000}
            bind:value={modifiedAgent.modelParameters.top_k}
        />
    </DetailViewSection>
    ...
    {#if id !== null}
        <DetailViewSection title="Delete agent">
            <DeleteButton aria-label="Delete agent" onclick={onDelete}/>
        </DetailViewSection>
        
        <DetailViewSection title="Save agent">
            <SaveButton aria-label="Save agent" onclick={onSave}/>
        </DetailViewSection>
    {:else}
        <DetailViewSection title="Create agent">
            <CreateButton aria-label="Create agent" onclick={onCreate}/>
        </DetailViewSection>
    {/if}
