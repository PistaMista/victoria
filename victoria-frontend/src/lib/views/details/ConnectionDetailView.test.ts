...
test('connection detail can edit URL of Ollama connection', async () => {
    ...
    let body = await (updateConnectionHandler as Mock).mock.calls[0][0].request.json();
...
test('connection detail can delete given Ollama connection', async () => {
    ...
    expect((deleteConnectionHandler as Mock).mock.calls[0][0].request.url).toContain('/api/connections/1');
})
...
test('connection detail can create new Ollama connection (given no id)', async () => {
    ...
    let body = await (createConnectionHandler as Mock).mock.calls[0][0].request.json();
