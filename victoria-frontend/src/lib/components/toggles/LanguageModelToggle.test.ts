...
test("clicking model toggle of disabled model sends enable request", async () => {
  ...
  expect((enableModelHandler as Mock).mock.calls[0][0].request.url).toContain(
    "/api/models/3/enable",
  );
});
...
test("clicking model toggle of enabled model sends disable request", async () => {
  ...
  expect((disableModelHandler as Mock).mock.calls[0][0].request.url).toContain(
    "/api/models/5/disable",
  );
});
