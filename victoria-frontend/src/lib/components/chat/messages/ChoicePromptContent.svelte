        ...
        {#if answer === null}
            <div class="flex flex-col space-y-2 rounded-md p-2 m-1 bg-slate-500">
                {#each content.choices as choice}
                    <Button 
                        class="w-full"
                        aria-label={JSON.stringify(choice.value)}
                        on:click={() => sendAnswerPromise = sendAnswer(choice.value)}
                    >{JSON.stringify(choice.value)}</Button>
                {/each}
            </div>
        {:else}
            <div>
                Chosen: <span class="font-bold">{JSON.stringify(answer)}</span>
            </div>
        {/if}
    </Loader>
    </Loader>
