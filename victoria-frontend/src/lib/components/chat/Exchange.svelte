<script lang="ts">
    ...
    import { duplicateChatToExchange, startReceivingMessages } from "$lib/api/chatting";
    ...
    async function loadMonologues() {
        monologues = await Promise.all(exchange.monologueIds.map(id => (getMonologue(id))));
    }

    function createNewChatFromHere() {
        createNewChatPromise = duplicateChatToExchange(exchange.chatId, exchange.id);
    }
    
    onMount(() => {
        getMonologuesPromise = loadMonologues();
        receivePromise = startReceivingMessages(exchange.id, agentMessages, abortController.signal);
    })
...
<div class="w-full flex flex-col">
    ...
    <div class="{exchange.userMessage ? "" : "rounded-t-md"} flex flex-col space-y-2 border-b-2 border-slate-500 border-dashed bg-slate-100 p-1">
