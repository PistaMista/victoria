...
class RunnerService:
    def __init__(
        ...
        thread_factory: Callable[[int, Callable[[MonologueThread], None]], MonologueThread],
        thread_limit: int
    ):
        self._db: DatabaseService = db_service
        self._limit: int = thread_limit
        self._thread_factory: Callable[[int, Callable[[MonologueThread], None]], MonologueThread] = thread_factory
    ...
    def start_monologue_process(self, monologue_id: int):
        ...
        with self._db.session() as db:
            monologue = db.scalar(
                select(Monologue).where(Monologue.id == monologue_id)
            )
    ...
    def on_thread_exited(self, t: MonologueThread):
        with self._db.session() as db:
            monologue = db.scalar(
                select(Monologue).where(Monologue.id == t._id)
            )
