...
class MonologueThread(threading.Thread):
    def __init__(
        self, monologue_id: int, on_finish: Callable[["MonologueThread"], None]
    ):
...
class AgenticMonologueThread(MonologueThread):
    def __init__(
        ...
        super().__init__(monologue_id=monologue_id, on_finish=on_finish)
    ...
    def do_iteration(self):
        ...
        new_thought = Thought(
            timestamp=datetime.now(tz=UTC), invocation=invocation, result=result
        )
