...
class MonologueService:
    ...
    def get_user_monologues(
        self,
        user_id: int,
        status_filter: Optional[MonologueStatus] = None,
        search_query: Optional[str] = None,
    ) -> List[Monologue]:
        """Gets all of the given User's monologues."""
        with self._db.session() as db:
            stmt = (
                select(Monologue).join(Monologue.agent).where(Agent.owner_id == user_id)
            )
    ...
    def end_user_monologue(self, user_id: int, monologue_id: int, successful: bool):
        ...
        with self._db.session() as db:
            ...
            self.set_monologue_status(
                ...
                if successful
                else MonologueStatus.FAILURE,
            )

    def get_user_monologue_thoughts(
        self, user_id: int, monologue_id: int
    ) -> List[Thought]:
    ...
    def get_monologue_thoughts(self, id: int):
        with self._db.session() as db:
            monologue = db.scalar(
                select(Monologue)
                .options(
                    joinedload(Monologue.thoughts).options(
    ...
    def get_monologue_by_id(self, id: int):
        with self._db.session() as db:
            monologue = db.scalar(
                select(Monologue)
                .where(Monologue.id == id)
                .options(joinedload(Monologue.agent))
            )
