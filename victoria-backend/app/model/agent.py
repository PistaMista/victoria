...
class Agent(Base):
    ...
    model_id: Mapped[int] = mapped_column(
        ForeignKey("language_model.id", ondelete="SET NULL"), nullable=True
    )
    ...
    allowed_triggers: Mapped[List["Trigger"]] = relationship(
        "Trigger",
        secondary=allowed_agent_trigger_association,
        back_populates="allowed_on_agents",
    )
    ...
    monologues: Mapped[List["Monologue"]] = relationship(
        "Monologue", back_populates="agent", cascade="all,delete"
    )
