...
class Trigger(Base):
    ...
    spawned_events: Mapped[List["Event"]] = relationship(
        "Event", back_populates="trigger"
    )
    allowed_on_agents: Mapped[List["Agent"]] = relationship(
        "Agent",
        secondary=allowed_agent_trigger_association,
        back_populates="allowed_triggers",
    )
    allowed_on_users: Mapped[List["User"]] = relationship(
        "User",
        secondary=allowed_user_trigger_association,
        back_populates="allowed_triggers",
    )
