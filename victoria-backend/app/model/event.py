...
class Event(Base):
    ...
    monologues: Mapped[List["Monologue"]] = relationship(
        "Monologue", back_populates="event"
    )

    triggering_chat_exchange_id: Mapped[int] = mapped_column(
        ForeignKey("chat_exchange.id"), nullable=True
    )
    triggering_chat_exchange: Mapped["ChatExchange"] = relationship(
        "ChatExchange", back_populates="triggered_chat_events"
    )
