...
class Action(Base):
    ...
    repository_id: Mapped[int] = mapped_column(
        ForeignKey("action_repository.id", ondelete="CASCADE"), nullable=False
    )
    repository: Mapped["ActionRepository"] = relationship(
        "ActionRepository", back_populates="actions"
    )

    allowed_on_agents: Mapped[List["Agent"]] = relationship(
        "Agent",
        secondary=allowed_agent_action_association,
        back_populates="allowed_actions",
    )
    allowed_on_chats: Mapped[List["Chat"]] = relationship(
        "Chat",
        secondary=allowed_chat_action_association,
        back_populates="allowed_actions",
    )
    allowed_on_users: Mapped[List["User"]] = relationship(
        "User",
        secondary=allowed_user_action_association,
        back_populates="allowed_actions",
    )
