...
class ChatMessage(Base):
    ...
    usermsg_exchange_id: Mapped[int] = mapped_column(
        ForeignKey("chat_exchange.id", ondelete="CASCADE"), nullable=True, unique=True
    )
    ...
    reply_exchange_id: Mapped[int] = mapped_column(
        ForeignKey("chat_exchange.id", ondelete="CASCADE"), nullable=True
    )
    reply_exchange: Mapped["ChatExchange"] = relationship(
        "ChatExchange", back_populates="agent_replies", foreign_keys=[reply_exchange_id]
    )
...
class ChatMessageChoicePrompt(ChatMessage):
    ...
    choices: Mapped[List["ChoiceMessageOption"]] = relationship(
        "ChoiceMessageOption", back_populates="message", cascade="all,delete"
    )
...
class ChoiceMessageOption(Base):
    ...
    message_id: Mapped[int] = mapped_column(
        ForeignKey("chat_message_prompt.id", ondelete="CASCADE"), nullable=False
    )
    message: Mapped[ChatMessageChoicePrompt] = relationship(
        "ChatMessageChoicePrompt", back_populates="choices"
    )
