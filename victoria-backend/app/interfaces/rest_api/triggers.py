...
async def get_allowed_triggers(
    ...
    allowed = trigger_service.get_user_allowed_triggers(user_id=user.id)
    ...
    match body.settings:
        case TimerSettings():
            created_trigger_id = trigger_service.add_timer_trigger(
                name=body.name, template=body.template, interval=body.settings.interval
            )
        case PollSettings():
        ...
        case ChatSettings():
            created_trigger_id = trigger_service.add_chat_trigger(
                name=body.name, template=body.template, receiver=body.settings.receiver
            )
        case WebhookSettings():
            created_trigger_id = trigger_service.add_webhook_trigger(
                name=body.name, template=body.template, endpoint=body.settings.url
            )
...
async def update_trigger(
    ...
    try:
        ...
        match (body.settings, existing):
            case (None, _):
                diff = TriggerDiff(name=body.name, template=body.template)
            case (TimerSettingsUpdate(type="timer"), _) | (
                TimerSettingsUpdate(),
                TimerTrigger(),
            ):
            ...
            case (PollSettingsUpdate(type="poll"), _) | (
                PollSettingsUpdate(),
                PollTrigger(),
            ):
            ...
            case (ChatSettingsUpdate(type="chat"), _) | (
                ChatSettingsUpdate(),
                ChatTrigger(),
            ):
            ...
            case (WebhookSettingsUpdate(type="webhook"), _) | (
                WebhookSettingsUpdate(),
                WebhookTrigger(),
            ):
        ...
        if diff is not None:
            trigger_service.update_trigger(trigger_id=id, changes=diff)
    except NonexistentTriggerError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
...
async def delete_trigger(
    ...
    try:
        trigger_service.remove_trigger(id=id)
    except NonexistentTriggerError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
