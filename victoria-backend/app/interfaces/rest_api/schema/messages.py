...
class MessageResponse(BaseModel):
    ...
    timestamp: int
    senderName: str
    content: Annotated[Union[MarkdownContent, ChoicePromptContent], Field(discriminator="type")]

def to_message_response(x: ChatMessage) -> MessageResponse:
    content = None
    if isinstance(x, ChatMessageMarkdown):
        content = MarkdownContent(
            markdownText=x.markdown
        )
    ...
    if content is None:
        content = MarkdownContent(
            markdownText="INVALID CONTENT TYPE, PLEASE FIX"
        )

    return MessageResponse(
        id=x.id,
        timestamp=int(x.timestamp.timestamp()),
        senderName=x.sending_user.username if x.sending_user
                   else x.sending_agent.name if x.sending_agent
                   else "Unknown",
        content=content
    )
