...
TriggerSettings = Union[TimerSettings, PollSettings, ChatSettings, WebhookSettings]
...
TriggerSettingsUpdate = Union[
    TimerSettingsUpdate, PollSettingsUpdate, ChatSettingsUpdate, WebhookSettingsUpdate
]
...
def to_trigger_response(x: Trigger) -> TriggerResponse:
    ...
    if isinstance(x, TimerTrigger):
        settings = TimerSettings(interval=x.interval)
    elif isinstance(x, PollTrigger):
        settings = PollSettings(interval=x.interval, url=x.url)
    elif isinstance(x, ChatTrigger):
        settings = ChatSettings(receiver=x.receiver)
    elif isinstance(x, WebhookTrigger):
        settings = WebhookSettings(url=x.endpoint)

    return TriggerResponse(
        id=x.id, name=x.name, template=x.template, parser="identity", settings=settings
    )
