...
def to_invocation_response(
    x: Optional[Invocation], event_id: int
) -> InvocationResponse:
    if x is None:
        return TriggerInvocationResponse(
            parameters=TriggerInvocationParams(eventId=event_id)
        )
    elif x.function_name == "think" and "content" in x.params:
        return ThoughtInvocationResponse(
            parameters=ThoughtInvocationParams(thought=x.params["content"])
        )
    elif (
        x.function_name == "end_monologue"
        and "successful" in x.params
        and x.params["successful"]
    ):
        return SuccessInvocationResponse(parameters=SuccessInvocationParams())
    elif (
        x.function_name == "end_monologue"
        and "successful" in x.params
        and not x.params["successful"]
    ):
        return FailureInvocationResponse(parameters=FailureInvocationParams())
    else:
        return ActionInvocationResponse(
            name=x.function_name or "INVALID", parameters=x.params or {}
        )
