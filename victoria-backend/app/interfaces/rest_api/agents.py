...
async def list_agents(
    ...
    return [
        AgentListItemResponse(
            ...
            name=x.name,
            status="BUSY"
            if agent_service.is_agent_running_monologues(x.id)
            else "IDLE",
        )
        for x in agents
    ]
...
async def get_agent(
    ...
    try:
        agent: Agent = agent_service.get_user_agent_by_id(user_id=user.id, agent_id=id)
        return AgentResponse(
            id=agent.id,
            name=agent.name,
            status="BUSY"
            if agent_service.is_agent_running_monologues(agent.id)
            else "IDLE",
...
async def update_agent(
    ...
    try:
        agent_service.update_user_agent(
            user_id=user.id, agent_id=id, diff=body.to_diff()
        )
    except InvalidAgentSettingError as err:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(err))
    except NonexistentAgentError as err:
...
async def delete_agent(
    ...
    try:
        agent_service.remove_user_agent(user_id=user.id, agent_id=id)
    except NonexistentAgentError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
