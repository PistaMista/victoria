...
from .schema.messages import (
    SendMessage,
    SendMessageChoicePrompt,
    SendMessageMarkdown,
    SentMarkdownInfoResponse,
    SentChoiceInfoResponse,
)
...
async def list_chats(
    ...
    chats = chat_service.get_user_chats(user_id=user.id)
...
async def create_chat(
    ...
    try:
        created_chat_id = chat_service.create_user_chat(user_id=user.id)
        created_chat = chat_service.get_user_chat(
            user_id=user.id, chat_id=created_chat_id
        )
...
async def list_receivers(
    ...
    return chat_service.get_user_chat_receivers(user_id=user.id)
...
async def get_chat_info(
    ...
    try:
        chat = chat_service.get_user_chat(user_id=user.id, chat_id=id)
...
async def delete_chat(
    ...
    try:
        chat_service.remove_user_chat(user_id=user.id, chat_id=id)
    except NonexistentChatError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
...
async def duplicate_chat(
    ...
    try:
        new_chat_id = chat_service.duplicate_user_chat(
            user_id=user.id, chat_id=id, last_exchange_id=body.toExchange
        )
        return new_chat_id
    except NonexistentChatError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
...
@router.post(
    "/{id}/send-message",
    response_model=Union[SentChoiceInfoResponse, SentMarkdownInfoResponse],
)
@inject
async def send_message_to_chat(
    ...
    try:
        if isinstance(msg, SendMessageMarkdown):
            ...
            return SentMarkdownInfoResponse(exchangeId=exchange_id)
            ...
            return SentChoiceInfoResponse(exchangeId=exchange_id, queryId=query_id)
    except NonexistentChatError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
    ...
    try:
        ...
        while not await request.is_disconnected() and running < timeout:
            new = chat_service.get_user_chat_exchanges_after(
                user_id=user.id, chat_id=id, after=after
            )
...
async def set_chat_options(
    ...
    try:
        chat_service.update_user_chat_options(
            user_id=user.id, chat_id=id, options=body.to_diff()
        )
    except NonexistentChatError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
...
async def set_chat_summary(
    ...
    try:
        chat_service.set_user_chat_summary(user_id=user.id, chat_id=id, summary=body)
    except NonexistentChatError as err:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(err))
...
async def get_chat_history(
    ...
    try:
        msgs = chat_service.get_user_chat_messages(user_id=user.id, chat_id=id)
        ...
        for msg in msgs:
            ...
            if isinstance(msg, ChatMessageMarkdown):
                if msg.sending_user is not None:
                    result.append(LLMUserMessage(message=msg.markdown))
                elif msg.sending_agent is not None:
                    result.append(
                        LLMAssistantMessage(
                            agentName=msg.sending_agent.name, message=msg.markdown
                        )
                    )
