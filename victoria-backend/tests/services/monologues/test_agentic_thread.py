...
def test_agentic_thread_calls_monologue_service_properly_on_iteration(thread_sut, mock_monologue_serv, mock_llm_serv):
    # Act
...
def test_agentic_thread_calls_llm_service_properly_on_iteration(thread_sut, mock_monologue_serv, mock_llm_serv):
    ...
    mock_llm_serv.get_chat_completion.assert_called_once_with(5, [
        SystemMessage("MY SYSTEM PROMPT"),
        UserMessage("An email has arrived"),
        AssistantMessage('{"action_name": "think", "arguments": {"content": "Lol"}}')        
    ])

def test_agentic_thread_calls_action_service_to_convert_llm_message_to_invocation_on_iteration(thread_sut, mock_llm_serv, mock_action_serv):
    # Act
    thread_sut.do_iteration()
...
def test_agentic_thread_stops_when_monologue_is_marked_as_succeeded(thread_sut, mock_monologue_serv, mock_on_finish):
    # Act
    thread_sut.start()
    mock_monologue_serv.get_monologue_by_id.return_value.status = MonologueStatus.SUCCESS
...
def test_agentic_thread_stops_when_monologue_is_marked_as_failed(thread_sut, mock_monologue_serv, mock_on_finish):
    # Act
    thread_sut.start()
    mock_monologue_serv.get_monologue_by_id.return_value.status = MonologueStatus.FAILURE
...
def test_agentic_thread_calls_on_finish_when_crashing_and_marks_monologue_as_failed(thread_sut, mock_monologue_serv, mock_on_finish):
    # Arrange
    ...
    # Assert
    mock_monologue_serv.set_monologue_status.assert_called_once_with(1, MonologueStatus.FAILURE)
    mock_on_finish.assert_called_once_with(thread_sut)
