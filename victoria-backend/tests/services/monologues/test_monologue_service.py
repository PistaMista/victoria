...
def sample_monologue(db_session, owner):
    ...
    action_repo = ActionRepository(
        name="Default", url="https://www.github.com/SOME_ACTION_REPO"
    )
    ...
    verbatim_invocation = Invocation(
        action=think_action,
        function_name="think",
        params={"content": "I should notify the user of the new email"},
    )
    ...
    action_invocation = Invocation(
        ...
        function_name="send_message",
        params={"exchange_id": 3, "message": "A new email has arrived"},
    )
    ...
    monologue = Monologue(
        ...
        context={},
        thoughts=[trigger_thought, verbatim_thought, action_thought],
    )
...
def test_monologue_service_can_list_user_monologues(db_serv, db_session):
    ...
    # Act
    res_all = service.get_user_monologues(user_id=1)
    res_running = service.get_user_monologues(
        user_id=1, status_filter=MonologueStatus.RUNNING
    )
    res_search = service.get_user_monologues(user_id=1, search_query="Search")
...
def test_monologue_service_returns_empty_list_when_listing_monologues_of_nonexistent_user(
    ...
    # Act
    res_all = service.get_user_monologues(user_id=99)
    res_running = service.get_user_monologues(
        user_id=99, status_filter=MonologueStatus.RUNNING
    )
    res_search = service.get_user_monologues(user_id=99, search_query="Search")
...
def test_monologue_service_can_end_user_monologue_with_success(
    ...
    service.end_user_monologue(
        sample_monologue.agent.owner_id, sample_monologue.id, True
    )

    # Assert
    mock_set_status.assert_called_once_with(
        id=sample_monologue.id, status=MonologueStatus.SUCCESS
    )
...
def test_monologue_service_can_end_user_monologue_with_failure(
    ...
    # Act
    service.end_user_monologue(
        sample_monologue.agent.owner_id, sample_monologue.id, False
    )

    # Assert
    mock_set_status.assert_called_once_with(
        id=sample_monologue.id, status=MonologueStatus.FAILURE
    )
...
def test_monologue_service_can_get_user_monologue_thoughts_including_invocations_and_monologues(
    ...
    res = service.get_user_monologue_thoughts(
        user_id=sample_monologue.agent.owner.id, monologue_id=sample_monologue.id
    )
...
def test_monologue_service_can_get_monologue_thoughts(
    ...
    assert (
        res[1].invocation.params["content"]
        == "I should notify the user of the new email"
    )
    assert res[1].result == "I should notify the user of the new email"
...
def test_monologue_service_can_get_monologue_thoughts_as_llm_chat_history(
    ...
    assert res == [
        ...
        AssistantMessage(
            '{"action_name": "think", "arguments": {"content": "I should notify the user of the new email"}}'
        ),
        UserMessage("I should notify the user of the new email"),
        AssistantMessage(
            '{"action_name": "send_message", "arguments": {"exchange_id": 3, "message": "A new email has arrived"}}'
        ),
