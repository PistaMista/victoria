...
def test_dispatcher_creates_new_monologue_when_an_event_is_added_to_db(
    ...
    trigger = (
        PollTrigger(  # A trigger generates events (by polling a website for example)
            name="Emails",
    ...
    assert (
        event.monologues[0].thoughts[0].result
...
def test_dispatcher_sets_monologue_agent_token_when_dispatching_monologue(
    mock_urandom, db_session, dispatcher, owner
):
    # Arrange
...
@mock.patch("app.services.monologues.dispatcher.os.urandom")
def test_dispatcher_generates_monologue_context_token_again_after_token_collision_when_dispatching_monologue(
    mock_urandom, db_session, dispatcher, owner
):
    # Arrange
    ...
    existing_event = Event(id=50, content="W", trigger=trigger, dispatched=True)
...
def test_dispatcher_instructs_runner_to_start_processing_when_monologue_is_added_to_db(
    dispatcher, runner, db_session, owner
):
    # Arrange
    trigger = PollTrigger(
        name="Emails",
...
def test_dispatcher_instructs_runner_to_process_existing_unfinished_monologues(
    ...
    action_repo = ActionRepository(name="Default", url="some_url")
    trigger_thought = Thought(
        timestamp=datetime.fromtimestamp(1),
        invocation=None,
        result="An email has arrived...",
    ...
    monologue = Monologue(
        ...
        agent=agent,
        status=MonologueStatus.PENDING,
        thoughts=[trigger_thought, verbatim_thought],
    )
    event.monologues = [monologue]
...
def test_dispatcher_instructs_runner_to_process_monologue_just_added_by_new_event(
    db_session, runner, dispatcher, owner
):
    # Arrange
    trigger = PollTrigger(
...
def test_dispatcher_does_nothing_for_finished_monologues(
    ...
    action_repo = ActionRepository(name="Default", url="lol.com")
    trigger_thought = Thought(
        timestamp=datetime.fromtimestamp(2),
        invocation=None,
        result="An email has arrived...",
    ...
    success_invocation = Invocation(
        ...
        function_name="mark_as_success",
        params={"reason": "The user has been notified"},
    )
    ...
    monologue = Monologue(
        ...
        agent=agent,
        thoughts=[trigger_thought, success_thought],
    )
    event.monologues = [monologue]
