...
from app.services.action.tool import (
    TOOL_REGISTRY,
    tool,
    UnannotatedActionParamError,
    InvalidActionParamTypeError,
)
...
def test_tool_decorator_registers_action_so_that_it_is_executable():
    ...
    namespace = {"tool": lambda x: x, "outer_func": outer_func}
...
def test_tool_decorator_registers_complex_action():
    ...
    assert TOOL_REGISTRY[0].function_param_schema == {"x": "int"}
...
def test_tool_decorator_registers_multiple_actions():
    ...
    assert TOOL_REGISTRY[0].function_param_schema == {"a": "float", "b": "int"}
    assert TOOL_REGISTRY[0].function_docstring == "Adds a and b."

    assert TOOL_REGISTRY[1].function_name == "sub"
    assert TOOL_REGISTRY[1].function_param_schema == {"a": "int", "b": "float"}
