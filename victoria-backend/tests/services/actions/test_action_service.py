...
from app.services.action import (
    ActionService,
    NonexistentActionError,
    NonexistentActionRepositoryError,
    ActionRepositoryDiff,
)
...
@mock.patch(
    "tests.services.actions.test_action_service.ActionService.set_action_repository_actions"
)
@mock.patch(
    "tests.services.actions.test_action_service.ActionService.import_actions_from_git_url"
)
def test_action_service_can_add_action_git_repository(
    mock_import_actions, mock_set_actions, db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
    mock_import_actions.return_value = []

    # Act
    created_id = serv.add_action_repository(
        "Home Assistant", "http://mygit.com/HATools"
    )

    # Assert
    repos = db_session.scalars(select(ActionRepository)).all()
    ...
    repo = db_session.scalar(
        select(ActionRepository).where(ActionRepository.id == created_id)
    )
...
def test_action_service_tries_to_import_and_set_actions_for_added_repository(
    mock_import_actions, mock_set_actions, db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
    ...
    # Assert
    repo = db_session.scalar(select(ActionRepository))
...
def test_action_service_tries_to_reimport_and_set_actions_for_existing_repositories_when_constructed(
    mock_import_actions, mock_set_actions, db_serv, db_session
):
    # Arrange
    repo_gitea = ActionRepository(id=600, name="Gitea", url="gitea")
    repo_gitlab = ActionRepository(id=700, name="GitLab", url="gitlab")
    ...
    # Act
    ActionService(database_service=db_serv)
...
def test_action_service_does_not_set_repository_actions_if_reimport_throws(
    mock_import_actions, mock_set_actions, db_serv, db_session
):
    # Arrange
    think_action = Action(
    ...
    repo_gitea = ActionRepository(
        id=600, name="Gitea", url="gitea", actions=[think_action, end_action]
    )
    db_session.add(repo_gitea)
    db_session.commit()
...
def test_action_service_can_set_actions_for_action_repository(db_serv, db_session):
    ...
    new_action = Action(
        function_name="new",
        function_param_schema={"iterations": "int"},
        function_docstring="LOL",
        function_source_code="",
    )
    existing_action = Action(
        function_name="end_workflow",
        function_param_schema={"new_param": "int"},
    ...
    actions = db_session.scalars(select(Action).order_by(Action.function_name)).all()
    ...
    assert actions[1].function_param_schema == {"iterations": "int"}
...
@mock.patch(
    "tests.services.actions.test_action_service.ActionService._clone_git_repository"
)
def test_action_service_imports_actions_from_simple_action_git_repository(
    mock_clone, db_serv, tmp_path
):
    # Arrange
    serv = ActionService(database_service=db_serv)
...
"""
    ...
    # Act
    actions = serv.import_actions_from_git_url(
        "https://github.com/PistaMista/victoria-default-actions"
    )
...
def test_action_service_can_remove_action_git_repository(
    mock_import_actions, db_serv, db_session
):
    # Arrange
    repo = ActionRepository(id=42, name="Gitea", url="gitea", actions=[])
    db_session.add(repo)
    db_session.commit()

    mock_import_actions.side_effect = (
        ConnectionError()
    )  # So that ActionService does not try to reimport the actions
    serv = ActionService(database_service=db_serv)
    ...
    # Assert
    repos = db_session.scalars(select(ActionRepository))
...
def test_action_service_deletes_actions_imported_from_removed_git_repository(
    ...
    repo = ActionRepository(
        id=42, name="Gitea", url="gitea", actions=[think_action, end_action]
    )
    db_session.add(repo)
    db_session.commit()
    ...
    # Assert
    actions = db_session.scalars(select(Action))
...
def test_action_service_can_get_action_descriptions(db_serv, db_session):
    ...
    repo = ActionRepository(
        name="Gitea", url="gitea", actions=[think_action, end_action]
    )
    db_session.add(repo)
    db_session.commit()
...
def test_action_service_can_get_all_action_repositories_and_their_actions(
    db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
    ...
    repo_github = ActionRepository(name="Github", url="github", actions=[search_action])
...
def test_action_service_can_get_action_repository_by_id_and_its_actions(
    db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
    ...
    repo_gitea = ActionRepository(
        id=32, name="Gitea", url="gitea", actions=[think_action, end_action]
    )
    repo_github = ActionRepository(
        id=40, name="Github", url="github", actions=[search_action]
    )
...
def test_action_service_can_update_action_repository(
    mock_import_actions, db_serv, db_session
):
    # Arrange
    mock_import_actions.return_value = []
    ...
    repo_gitea = ActionRepository(id=32, name="Gitea", url="gitea", actions=[])
    repo_github = ActionRepository(id=40, name="Github", url="github", actions=[])
...
@mock.patch(
    "tests.services.actions.test_action_service.ActionService._reimport_actions_for_existing_repositories"
)
def test_action_service_update_action_repository_triggers_action_reimport(
    mock_reimport, db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
...
def test_action_service_returns_empty_list_when_getting_allowed_actions_for_nonexistent_user(
    db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
...
def test_action_service_parses_valid_invocation_json(db_serv, db_session):
    ...
    assert res.params == {"successful": True, "reason": "Replied to user greeting."}
...
def test_action_service_parses_syntactically_invalid_invocation_json(
    ...
    repo = ActionRepository(name="Gitea", url="gitea", actions=[end_action])
    db_session.add(repo)
    db_session.commit()
...
def test_action_service_parses_semantically_invalid_invocation_json_with_missing_arguments(
    ...
    assert res.action.id == end_action.id
    assert res.function_name == "end_workflow"
    assert res.params == {"successful": True}
    assert res.function_name_semantically_valid
    assert not res.params_semantically_valid
...
def test_action_service_parses_semantically_invalid_invocation_json_with_extraneous_arguments(
    ...
    assert res.action.id == end_action.id
    assert res.function_name == "end_workflow"
    assert res.params == {"successful": True, "reason": "Reason!", "id": 40}
    assert res.function_name_semantically_valid
    assert not res.params_semantically_valid
...
def test_action_service_parses_semantically_invalid_invocation_json_with_invalid_argument_types(
    ...
    assert res.action.id == end_action.id
    assert res.function_name == "end_workflow"
    assert res.params == {"successful": True, "reason": 3.14}
    assert res.function_name_semantically_valid
    assert not res.params_semantically_valid
...
def factorial(x: int, **kwargs):
    ...
    repo = ActionRepository(name="Gitea", url="gitea", actions=[factorial_action])
    db_session.add(repo)
    db_session.commit()
...
def test_action_service_allows_executed_action_to_access_and_modify_provided_context(
    db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
...
def increment(**kwargs):
    ...
    repo = ActionRepository(name="Gitea", url="gitea", actions=[increment_action])
    db_session.add(repo)
    db_session.commit()
...
def test_action_service_throws_when_trying_to_manipulate_nonexistent_action(
    db_serv, db_session
):
    # Arrange
    serv = ActionService(database_service=db_serv)
