...
from app.services.llm import (
    ...
    NonexistentModelError,
    DisabledModelError,
    OllamaConnectionDiff,
)
...
def test_llm_service_can_get_all_enabled_models(mock_get, db_serv, db_session):
    # Arrange
    serv = LLMService(database_service=db_serv)

    connection1 = OllamaConnection(
        name="Megacenter",
        url="http://localhost:11434",
        models=[
            LanguageModel(id=1, name="gemma3:12b", enabled=True),
            LanguageModel(id=2, name="deepseek-r1:latest", enabled=False),
        ],
    )
    connection2 = OllamaConnection(
        ...
        models=[
            LanguageModel(id=3, name="qwen2", enabled=False),
            LanguageModel(id=4, name="gpt-oss", enabled=False),
        ],
    )
...
def test_llm_service_can_disable_and_enable_models(mock_get, db_serv, db_session):
    ...
    to_enable = LanguageModel(id=2, name="deepseek-r1:latest", enabled=False)
    to_disable = LanguageModel(id=3, name="qwen2", enabled=True)
    connection1 = OllamaConnection(
        name="Megacenter",
        url="http://localhost:11434",
        models=[LanguageModel(id=1, name="gemma3:12b", enabled=True), to_enable],
    )
    connection2 = OllamaConnection(
        name="ChatGPT",
        url="http://chatgpt.com",
        models=[LanguageModel(id=4, name="gpt-oss", enabled=False), to_disable],
    )
...
def test_llm_service_can_update_ollama_connection(mock_get, db_serv, db_session):
    ...
    serv.update_ollama_connection(
        id=1, changes=OllamaConnectionDiff(name="MINE", url="woo")
    )
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_reimports_models_from_updated_ollama_connection(
    mock_get, db_serv, db_session
):
    # Arrange
    ...
    connection1 = OllamaConnection(
        ...
        url="http://localhost:11434",
        models=[
            LanguageModel(id=10, name="gemma3:12b", enabled=True),
            LanguageModel(id=20, name="deepseek-r1:latest", enabled=False),
        ],
    )
    connection2 = OllamaConnection(
        id=2,
        ...
        url="http://chatgpt.com",
        models=[
            LanguageModel(id=30, name="qwen2", enabled=False),
            LanguageModel(id=40, name="gpt-oss", enabled=False),
        ],
    )
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_fails_to_update_ollama_connection_on_ollama_error_status_code(
    mock_get, db_serv, db_session
):
    # Arrange
    mock_res = mock_get.return_value
    ...
    with pytest.raises(OllamaCommunicationError):
        serv.update_ollama_connection(
            id=1, changes=OllamaConnectionDiff(name="WOOF", url="nope")
        )
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_fails_to_update_ollama_connection_on_ollama_server_connection_error(
    mock_get, db_serv, db_session
):
    # Arrange
...
def test_llm_service_can_add_a_new_ollama_connection(mock_get, db_serv, db_session):
    ...
    # Act
    res_id = serv.add_ollama_connection(name="Homelab", url="http://localhost:11434")

    # Assert
    connections = db_session.scalars(select(LLMConnection)).all()
...
def test_llm_service_imports_models_from_added_ollama_connection(
    ...
    connection = db_session.scalar(
        select(LLMConnection).where(LLMConnection.id == res_id)
    )

    models = db_session.scalars(
        select(LanguageModel).order_by(LanguageModel.name)
    ).all()
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_fails_to_add_ollama_connection_on_ollama_error_status_code(
    mock_get, db_serv, db_session
):
    # Arrange
    mock_res = mock_get.return_value
    ...
    # Act / Assert
    with pytest.raises(OllamaCommunicationError):
        serv.add_ollama_connection(name="Homelab", url="http://localhost:11434")

    connections = db_session.scalars(select(LLMConnection))
    models = db_session.scalars(select(LanguageModel))
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_fails_to_add_ollama_connection_on_ollama_server_connection_error(
    mock_get, db_serv, db_session
):
    # Arrange
    mock_get.side_effect = ConnectionError()
...
def test_llm_service_imports_models_from_existing_connections_on_startup(
    ...
    # Arrange
    connection = OllamaConnection(name="Megacenter", url="http://localhost:11434")
...
def test_llm_service_overrides_models_in_database_with_models_from_existing_connection(
    ...
    replaced_model = LanguageModel(
        id=4, name="gemma3:12b", enabled=True, connection=connection
    )
    untouched_model = LanguageModel(
        id=42, name="deepseek-r1:latest", enabled=True, connection=connection
    )
...
def test_llm_service_keeps_models_in_database_on_ollama_communication_failure(
    ...
    # Arrange
    connection = OllamaConnection(name="Megacenter", url="http://localhost:11434")
    model = LanguageModel(name="gemma3:12b", enabled=True, connection=connection)
...
def test_llm_service_can_remove_an_ollama_connection(mock_get, db_serv, db_session):
    ...
    connection = OllamaConnection(
        id=10, name="Megacenter", url="http://www.megacenter.com:11434"
    )
    model = LanguageModel(name="gemma3:12b", enabled=True, connection=connection)
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_uses_ollama_api_correctly_for_chat_completion(
    mock_get, mock_post, db_serv, db_session
):
    # Arrange
    connection = OllamaConnection(
        id=10, name="Megacenter", url="http://www.megacenter.com:11434"
    )
    model = LanguageModel(id=3, enabled=True, name="gemma3:12b", connection=connection)
    ...
    mock_post_res.json.return_value = {
        ...
        "message": {"role": "assistant", "content": "Here is a recipe..."},
        "done": True,
    ...
    mock_post.assert_called_once_with(
        ...
        json={
            ...
            "stream": False,
            "messages": [
                {"role": "system", "content": "YOU ARE THE ONE NEO!"},
                {"role": "user", "content": "Hello there!"},
                {"role": "assistant", "content": "What can I help you with?"},
                {"role": "user", "content": "Give me a recipe for fish and chips."},
            ],
        },
    )
...
@mock.patch("app.services.llm.requests.post")
@mock.patch("app.services.llm.requests.get")
def test_llm_service_throws_on_communication_failure_during_chat_completion(
    mock_get, mock_post, db_serv, db_session
):
    # Arrange
    connection = OllamaConnection(
...
def test_llm_service_throws_when_using_disabled_model_for_chat_completion(
    ...
    model = LanguageModel(id=3, enabled=False, name="gemma3:12b", connection=connection)
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_throws_when_trying_to_manipulate_nonexistent_model(
    mock_get, db_serv, db_session
):
    # Arrange
    serv = LLMService(database_service=db_serv)
...
@mock.patch("app.services.llm.requests.get")
def test_llm_service_throws_when_trying_to_manipulate_nonexistent_connection(
    mock_get, db_serv, db_session
):
    # Arrange
    serv = LLMService(database_service=db_serv)
