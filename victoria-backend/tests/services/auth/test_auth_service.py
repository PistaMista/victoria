...
def auth_serv():
    ...
    user_serv_mock.get_user_by_running_monologue_agent_token.side_effect = (
        get_user_by_agent_token_mock
    )
...
def test_auth_service_returns_jwt_token_with_valid_login(time, auth_serv):
    ...
    # Act
    token = auth_serv.get_login_token(username="John", password="actual")

    # Assert
    assert (
        token
        == "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxLCJpc3N1ZWQiOjE3NTMyMTEwMzYsImV4cGlyZXMiOjE3NTU4MDMwMzZ9.SO2nJ4lBOv-kVBpw8iZOHIsg_idRrNQDc1ufU8VKPmI"
    )
...
def test_auth_service_raises_error_when_getting_token_with_invalid_login(
    auth_serv, username, password
):
    # Arrange

    # Act / Assert
    with pytest.raises(InvalidLoginError):
        auth_serv.get_login_token(username=username, password=password)
