...
def test_list_users_returns_200_and_list_of_users_on_valid_request(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    ...
    user_mock.get_all_users.return_value = [
        User(id=2, username="krystof", role=Role.ADMIN),
        User(id=3, username="john", role=Role.USER),
    ]
    ...
    assert res.json() == [
        {"id": 2, "username": "krystof", "role": "admin"},
        {"id": 3, "username": "john", "role": "user"},
    ]
...
def test_create_user_returns_200_and_creates_user_on_valid_request(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"id": 4, "username": "mark", "role": "admin"}


def test_create_user_returns_400_on_username_taken(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_create_user_returns_400_on_specifying_invalid_settings(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_create_user_returns_403_when_not_admin(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_get_user_returns_200_and_user_on_valid_request(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    user_mock.get_user_by_id.return_value = User(
        ...
        role=Role.ADMIN,
        allowed_actions=[Action(id=20), Action(id=40)],
        allowed_triggers=[PollTrigger(id=42), PollTrigger(id=60)],
    )

    # Act
    res = mock_client.get("/api/users/3")

    # Assert
    user_mock.get_user_by_id.assert_called_with(id=3)
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {
        "id": 3,
        "username": "john",
...
def test_update_user_returns_200_and_updates_user_on_valid_request(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    res = mock_client.put(
        "/api/users/3",
        json={"newPassword": "weee", "role": "user", "permittedActions": [98, 56, 9]},
    )
    ...
    user_mock.update_user.assert_called_with(
        id=3,
        changes=UserDiff(
            new_password="weee", role=Role.USER, permitted_action_ids=[98, 56, 9]
        ),
    )
    assert res.status_code == status.HTTP_200_OK


def test_update_user_returns_400_on_username_taken(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_update_user_returns_400_on_specifying_invalid_ids(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_update_user_returns_403_when_not_admin(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_update_user_returns_404_for_nonexistent_user(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_delete_user_returns_200_and_deletes_user_for_valid_request(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Assert
    user_mock.delete_user_by_id.assert_called_with(id=42)
    assert res.status_code == status.HTTP_200_OK
...
def test_delete_user_returns_403_when_not_admin(
    mock_client, auth_mock, user_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
