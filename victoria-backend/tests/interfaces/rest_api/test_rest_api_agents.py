...
def test_list_agents_returns_200_and_list_of_agents_of_current_user_on_valid_request(
    ...
    agent_mock.get_user_agents.return_value = [
        Agent(id=1, name="Cook"),
        Agent(id=2, name="Researcher"),
        Agent(id=3, name="Maintainer"),
    ]

    # Act
    res = mock_client.get("/api/agents", cookies={"token": "tok"})
    ...
    assert res.json() == [
        {"id": 1, "name": "Cook", "status": "BUSY"},
        {"id": 2, "name": "Researcher", "status": "IDLE"},
        {"id": 3, "name": "Maintainer", "status": "BUSY"},
    ]
...
def test_create_agent_returns_200_and_creates_agent_for_current_user_on_valid_request(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    res = mock_client.post(
        ...
        json={
            ...
            "modelParameters": {"temperature": 0.7, "top_k": 0.95},
    ...
    agent_mock.add_user_agent.assert_called_with(
        ...
        model_parameters={"temperature": 0.7, "top_k": 0.95},
...
def test_create_agent_returns_400_on_invalid_agent_setting(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_create_agent_returns_401_when_not_signed_in(
    mock_client, auth_mock, agent_mock
):
    # Arrange
    auth_mock.get_as_non_admin_user.side_effect = NotLoggedInError()
...
def test_get_agent_returns_200_and_agent_on_valid_request(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    agent_mock.get_user_agent_by_id.return_value = Agent(
        ...
        model_id=20,
        model=LanguageModel(id=20, name="gemma3:12b", enabled=True),
...
def test_get_agent_returns_404_for_nonexistent_agent(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_update_agent_returns_200_and_updates_agent_on_simple_request(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.put("/api/agents/3", json={"name": "New", "baseModelId": 8})

    # Assert
    agent_mock.update_user_agent.assert_called_with(
        user_id=1, agent_id=3, diff=AgentDiff(name="New", model_id=8)
    )
    assert res.status_code == status.HTTP_200_OK


def test_update_agent_returns_200_and_updates_agent_on_complex_request(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    res = mock_client.put(
        ...
        json={
            "name": "New",
            "baseModelId": 8,
            "modelParameters": {"LOLOL": 420, "top_dog": 1337},
    ...
    agent_mock.update_user_agent.assert_called_with(
        ...
        diff=AgentDiff(
            name="New",
            model_id=8,
            model_params={"LOLOL": 420, "top_dog": 1337},
...
def test_update_agent_returns_400_when_using_undefined_ids(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_update_agent_returns_401_when_not_signed_in(
    mock_client, auth_mock, agent_mock
):
    # Arrange
    auth_mock.get_as_non_admin_user.side_effect = NotLoggedInError()
...
def test_update_agent_returns_404_for_nonexistent_agent(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.put("/api/agents/3", json={})

    # Assert
    agent_mock.update_user_agent.assert_called_with(
        user_id=1, agent_id=3, diff=AgentDiff()
    )
    assert res.status_code == status.HTTP_404_NOT_FOUND


def test_delete_agent_returns_200_and_deletes_agent_on_valid_request(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.delete("/api/agents/3", cookies={"token": "tok"})

    # Assert
    agent_mock.remove_user_agent.assert_called_with(user_id=1, agent_id=3)
    assert res.status_code == status.HTTP_200_OK


def test_delete_agent_returns_401_when_not_logged_in(
    mock_client, auth_mock, agent_mock
):
    # Arrange
    auth_mock.get_as_non_admin_user.side_effect = NotLoggedInError()
...
def test_delete_agent_returns_404_for_nonexistent_agent(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_list_agent_monologues_returns_200_and_monologue_list_on_valid_request(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.get("/api/agents/3/monologues", cookies={"token": "tok"})

    # Assert
    agent_mock.get_user_agent_monologues.assert_called_with(user_id=1, agent_id=3)
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == [
...
def test_list_agent_monologues_returns_401_when_not_signed_in(
    mock_client, auth_mock, agent_mock
):
    # Arrange
    auth_mock.get_as_non_admin_user.side_effect = NotLoggedInError()
...
def test_list_agent_monologues_returns_404_for_nonexistent_agent(
    mock_client, auth_mock, agent_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
