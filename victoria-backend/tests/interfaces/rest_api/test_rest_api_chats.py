...
def test_list_chats_returns_200_and_list_of_chats_for_current_user_on_valid_request(
    ...
    chat_mock.get_user_chats.return_value = [
        Chat(id=1, title="System admin", summary="Chat about system administration."),
        Chat(
            id=2,
    ...
    # Act
    res = mock_client.get("/api/chats", cookies={"token": "tok"})

    # Assert
    chat_mock.get_user_chats.assert_called_with(user_id=1)
...
def test_create_chat_returns_200_and_creates_chat_on_valid_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    chat_mock.get_user_chat.return_value = Chat(
        id=3, title="New chat", summary="No summary"
    )

    # Act
    res = mock_client.post("/api/chats", cookies={"token": "tok"})

    # Assert
    chat_mock.create_user_chat.assert_called_with(user_id=1)
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"id": 3, "title": "New chat", "summary": "No summary"}


def test_create_chat_returns_400_and_when_user_does_not_exist(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    chat_mock.create_user_chat.side_effect = CannotCreateChatForNonexistentUserError(
        user.id
    )
...
def test_delete_chat_returns_200_and_deletes_chat_on_valid_request(
    ...
    # Act
    res = mock_client.delete("/api/chats/57", cookies={"token": "tok"})

    # Assert
    chat_mock.remove_user_chat.assert_called_with(user_id=1, chat_id=57)
    assert res.status_code == status.HTTP_200_OK
...
def test_list_receivers_returns_200_and_list_of_receivers_for_current_user_on_valid_request(
    ...
    # Act
    res = mock_client.get("/api/chats/receivers", cookies={"token": "tok"})
...
def test_duplicate_chat_returns_200_and_duplicates_chat_on_valid_request(
    ...
    # Act
    res = mock_client.post("/api/chats/42/duplicate", json={}, cookies={"token": "tok"})

    # Assert
    assert res.status_code == status.HTTP_200_OK
    chat_mock.duplicate_user_chat.assert_called_with(
        user_id=1, chat_id=42, last_exchange_id=None
    )
    assert res.json() == 75


def test_duplicate_chat_returns_200_and_duplicates_chat_up_to_exchange_on_valid_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    res = mock_client.post(
        "/api/chats/42/duplicate", json={"toExchange": 101}, cookies={"token": "tok"}
    )

    # Assert
    chat_mock.duplicate_user_chat.assert_called_with(
        user_id=1, chat_id=42, last_exchange_id=101
    )
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == 75
...
def test_send_message_to_chat_returns_200_and_creates_exchange_with_markdown_message_on_user_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.post(
        "/api/chats/4/send-message", json={"type": "markdown", "message": "Message!"}
    )

    # Assert
    chat_mock.send_markdown_message_to_user_chat.assert_called_with(
        user_id=1, chat_id=4, from_agent_id=None, markdown="Message!"
    )
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"exchangeId": 10}


def test_send_message_to_chat_returns_200_and_creates_exchange_with_markdown_message_on_agent_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    res = mock_client.post(
        ...
        json={"type": "markdown", "message": "Message!", "fromAgentId": 33},
    )

    # Assert
    chat_mock.send_markdown_message_to_user_chat.assert_called_with(
        user_id=1, chat_id=4, from_agent_id=33, markdown="Message!"
    )
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"exchangeId": 10}


def test_send_message_to_chat_returns_200_creates_exchange_with_choice_prompt_and_returns_query_id_on_user_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"exchangeId": 42, "queryId": 120}


def test_send_message_to_chat_returns_200_creates_exchange_with_choice_prompt_and_returns_query_id_on_agent_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_send_message_to_chat_returns_422_when_type_unspecified_on_user_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.post("/api/chats/4/send-message", json={"message": "Weee"})
...
def test_send_message_to_chat_returns_422_when_unknown_message_type_specified(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.post(
        "/api/chats/4/send-message", json={"type": "something", "message": "Weee"}
    )
...
def test_send_message_to_chat_returns_404_for_nonexistent_chat(
    ...
    res1 = mock_client.post(
        "/api/chats/4/send-message", json={"type": "markdown", "message": "Weee"}
    )
    res2 = mock_client.post(
        "/api/chats/4/send-message",
        json={"type": "choice_prompt", "prompt": "Pick", "choices": [10, 20]},
    )

    # Assert
    chat_mock.send_markdown_message_to_user_chat.assert_called_with(
        user_id=1, chat_id=4, from_agent_id=None, markdown="Weee"
    )
    chat_mock.send_choice_message_to_user_chat.assert_called_with(
        user_id=1, chat_id=4, from_agent_id=None, prompt="Pick", choices=[10, 20]
    )
...
def test_list_exchanges_returns_200_and_list_of_exchanges_when_new_exchanges_available(
    ...
    # Act
    res = mock_client.get("/api/chats/4/exchanges", params={"after": 1000})

    # Assert
    assert res.status_code == status.HTTP_200_OK
    chat_mock.get_user_chat_exchanges_after.assert_called_with(
        user_id=1, chat_id=4, after=1000
    )
    assert res.json() == [
        {
            "id": 2,
    ...
    def data_source():
        ...
        chat_mock.get_user_chat_exchanges_after.return_value = [
            ChatExchange(
                ...
                user_message=None,
                triggered_chat_events=[Event(monologues=[Monologue(id=320)])],
            )
        ]
...
def test_list_exchanges_returns_200_and_empty_list_on_timeout(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
...
def test_get_chat_options_returns_200_and_chat_options_on_valid_request(
    ...
    chat_mock.get_user_chat.return_value = Chat(
        ...
        summary="Summary",
        receiver="general",
        allowed_actions=[Action(id=15), Action(id=90)],
    )

    # Act
    res = mock_client.get("/api/chats/4/options")

    # Assert
    chat_mock.get_user_chat.assert_called_with(user_id=1, chat_id=4)
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"receiver": "general", "enabledActionIds": [15, 90]}
...
def test_set_chat_options_returns_200_and_updates_chat_options_on_simple_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.put("/api/chats/4/options", json={"receiver": "new"})

    # Assert
    chat_mock.update_user_chat_options.assert_called_with(
        user_id=1, chat_id=4, options=ChatOptionsDiff(receiver="new")
    )
    assert res.status_code == status.HTTP_200_OK


def test_set_chat_options_returns_200_and_updates_chat_options_on_complex_request(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.put(
        "/api/chats/4/options", json={"receiver": "new", "enabledActionIds": [84, 24]}
    )
    ...
    chat_mock.update_user_chat_options.assert_called_with(
        user_id=1,
        chat_id=4,
        options=ChatOptionsDiff(receiver="new", enabled_action_ids=[84, 24]),
    )
    assert res.status_code == status.HTTP_200_OK
...
def test_set_chat_options_returns_404_for_nonexistent_chat(
    ...
    # Act
    res = mock_client.put("/api/chats/4/options", json={"receiver": "blabla"})

    # Assert
    chat_mock.update_user_chat_options.assert_called_with(
        user_id=1, chat_id=4, options=ChatOptionsDiff(receiver="blabla")
    )
    assert res.status_code == status.HTTP_404_NOT_FOUND


def test_set_chat_summary_returns_200_and_set_chat_summary_for_valid_requests(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.post("/api/chats/50/summary", json="My new summary!")

    # Assert
    chat_mock.set_user_chat_summary.assert_called_with(
        user_id=1, chat_id=50, summary="My new summary!"
    )
    assert res.status_code == status.HTTP_200_OK
...
def test_set_chat_summary_returns_401_when_not_logged_in(
    ...
    # Act
    res = mock_client.post("/api/chats/50/summary", json="Lol")
...
def test_set_chat_summary_returns_404_for_nonexistent_chat(
    ...
    # Assert
    chat_mock.set_user_chat_summary.assert_called_with(
        user_id=1, chat_id=50, summary="Lol"
    )
    assert res.status_code == status.HTTP_404_NOT_FOUND


def test_get_chat_history_returns_200_and_all_chat_messages_for_valid_requests(
    mock_client, auth_mock, chat_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
    ...
    # Act
    res = mock_client.get("/api/chats/42/history")

    # Assert
    chat_mock.get_user_chat_messages.assert_called_with(user_id=1, chat_id=42)
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == [
        ...
        {"role": "user", "message": "Give me a recipe for pretzels"},
        {"role": "assistant", "agentName": "Cook", "message": "Here it is"},
    ]
...
def test_get_chat_returns_200_and_basic_chat_info_on_valid_request(
    ...
    # Act
    res = mock_client.get("/api/chats/42")

    # Assert
    chat_mock.get_user_chat.assert_called_with(user_id=1, chat_id=42)
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"id": 3, "title": "Dogs", "summary": "Chat about dogs"}
