...
def test_get_event_returns_200_and_event_on_valid_request(
    ...
    trigger_mock.get_user_event.return_value = Event(
        trigger=PollTrigger(
            id=10, name="lol", template="temp", url="seznam.cz", interval=20
        ),
        trigger_id=10,
        content="Weee",
        dispatched=False,
    ...
    # Assert
    trigger_mock.get_user_event.assert_called_with(user_id=1, event_id=5)
    assert res.status_code == status.HTTP_200_OK
    assert res.json() == {"triggerId": 10, "content": "Weee"}
...
def test_get_event_returns_404_for_nonexistent_event(
    mock_client, auth_mock, trigger_mock, user
):
    # Arrange
    auth_mock.get_as_non_admin_user.return_value = user
