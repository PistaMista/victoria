...
def test_login_calls_auth_service_correctly(mock_client, auth_mock):
    ...
    # Act
    mock_client.post(
        "/api/auth/login", json={"username": "usernamos", "password": "passwordito"}
    )

    # Assert
    auth_mock.get_login_token.assert_called_once_with("usernamos", "passwordito")
...
def test_register_creates_admin_user_when_no_user_exists(mock_client, user_mock):
    ...
    # Act
    mock_client.post(
        "/api/auth/register", json={"username": "usernamos", "password": "passwordito"}
    )

    # Assert
    user_mock.create_user.assert_called_once_with(
        username="usernamos", password="passwordito", role=Role.ADMIN
    )
...
def test_register_creates_normal_user_when_another_user_exists(mock_client, user_mock):
    ...
    # Assert
    user_mock.create_user.assert_called_once_with(
        username="usernamos", password="passwordito", role=Role.USER
    )
...
def test_me_calls_auth_service_correctly(mock_client, auth_mock):
    ...
    # Act
    try:
        mock_client.get("/api/auth/me", cookies={"token": "tokenito"})
    except:
        pass
...
def test_me_admin_calls_auth_service_correctly(mock_client, auth_mock):
    ...
    # Act
    try:
        mock_client.get("/api/auth/me/admin", cookies={"token": "tokenito"})
    except:
        pass
